...
@final
class McpAdminLocMysqlController:
    """MCP Server controller for local MySQL administration."""
    ...
    async def __call__(self) -> None:
        async with stdio_server() as (read_stream, write_stream):
            self._logger.write_info(
                module="McpAdminLocMysqlController",
                message="Starting MCP Admin Loc MySQL server",
            )
        ...
        async def list_tools() -> list[Tool]:
            try:
                ...
                self.__log_exception(
                    module="McpAdminLocMysqlController.list_tools",
                    base_exception=e,
                )
                return []
...
def start_mcp_or_fail() -> None:
    """Start the MCP server for local MySQL administration."""
    try:
        asyncio.run(McpAdminLocMysqlController.get_instance()())
    except BaseException as error:
        Logger.get_instance().write_error(
            module="McpAdminLocMysqlController.start_mcp_or_fail",
            message=(
