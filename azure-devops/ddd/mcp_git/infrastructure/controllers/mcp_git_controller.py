...
from ddd.mcp_git.domain.enums import McpServerNameEnum
from ddd.mcp_git.application import CallToolDto, ListToolsService, CallToolService


@final
class McpGitController:
    """MCP Server controller for local git integration-branch operations."""
    ...
    def __init__(self) -> None:
        ...
        self._mcp_server = Server(McpServerNameEnum.GIT.value)
    ...
    def __register_services_as_handlers(self) -> None:
        ...
        async def list_tools() -> list[Tool]:
            try:
                ...
                self.__log_exception(
                    module="mcp_git_controller._register_handlers.list_tools",
                    base_exception=e
                )
                return []
        ...
        async def call_tool(event_name: str, payload_dict: dict) -> list[TextContent]:
            try:
                ...
                self.__log_exception(
                    module="mcp_git_controller._register_handlers.call_tool",
                    base_exception=e,
...
def start_mcp_or_fail() -> None:
    """Start the MCP server for local git integration-branch operations."""
    try:
        asyncio.run(McpGitController.get_instance()())
    except BaseException as error:
        Logger.get_instance().write_error(
            module="mcp_git_controller.start_mcp",
            message=(
