...
@final
class McpAnubisController:
    """MCP Server controller for Anubis API operations."""
    ...
    def __init__(self) -> None:
        ...
        self._mcp_server = Server(McpServerNameEnum.ANUBIS.value)
    ...
    async def __call__(self) -> None:
        async with stdio_server() as (mcp_read_stream, mcp_write_stream):
            self._logger.write_info(
                module="mcp_anubis_controller", message="__call__"
            )
    ...
    def __register_services_as_handlers(self) -> None:
        self._logger.write_info(
            module="mcp_anubis_controller", message="_register_handlers"
        )
        ...
        async def list_tools() -> list[Tool]:
            try:
                ...
                self.__log_exception(
                    module="mcp_anubis_controller._register_handlers.list_tools",
                    base_exception=e,
                )
                return []
...
def start_mcp_or_fail() -> None:
    """Start the MCP server for Anubis API operations."""
    try:
        asyncio.run(McpAnubisController.get_instance()())
    except BaseException as error:
        Logger.get_instance().write_error(
            module="mcp_anubis_controller.start_mcp",
            message=(
