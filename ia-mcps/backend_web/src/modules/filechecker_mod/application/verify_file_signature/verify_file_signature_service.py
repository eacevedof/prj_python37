...
from src.modules.filechecker_mod.application.verify_file_signature.verify_file_signature_dto import VerifyFileSignatureDto
from src.modules.filechecker_mod.application.verify_file_signature.verify_file_signature_result_dto import VerifyFileSignatureResultDto
from src.modules.filechecker_mod.domain.enums import (
...
from src.modules.filechecker_mod.infrastructure.repositories.malware_threat_intelligence_repository import (
...
class VerifyFileSignatureService:
    ...
    def __call__(
        ...
        if is_url:
            # Interruptor: la URL la elige un modelo, así que un prompt inyectado
            # puede apuntar la descarga a donde alcance la red del proceso (aquí,
            # tu LAN y tu localhost). Viene apagada de fábrica y este repo la
            # enciende en su .env por ser de uso local; ver el to-do de prompt
            # injection antes de exponer el proceso a una red ajena.
            if not self._environment_reader_raw_repository.is_file_checker_url_download_allowed():
                FileCheckerException.bad_request_custom(
                    "VerifyFileSignatureService: url download is disabled; "
                    "pass a local file path instead"
                )
