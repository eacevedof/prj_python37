...
from src.modules.pdf_mod.infrastructure.repositories.abstract_to_pdf_file_repository import AbstractToPdfFileRepository
...
class MdToPdfWriterFileRepository(AbstractToPdfFileRepository):
    ...
    def __call__(self, convert_md_to_pdf_dto: ConvertMdToPdfDto) -> ConvertMdToPdfResultDto:
        # BUG heredado: llamaba a `self._logger.info(...)`, que NO existe en
        # Logger (es `log_info(module, message)`) -> AttributeError en la primera
        # línea de toda conversión.
        self._logger.log_info(
            module="md_to_pdf_writer_file_repository",
            message=f"converting {convert_md_to_pdf_dto.md_file_path}",
        )

        md_content = self._read_md_file(convert_md_to_pdf_dto.md_file_path)
        # markdown y weasyprint se importan AQUÍ, no arriba: weasyprint arrastra
        # librerías nativas (cairo/pango) y con el import a nivel de módulo, una
        # máquina sin ellas tumbaba el arranque de TODA la app — incluidos los
        # servidores MCP que no tienen nada que ver con PDF.
        import markdown
        ...
        self._logger.log_info(
            module="md_to_pdf_writer_file_repository",
            message=(
                f"PDF written to {convert_md_to_pdf_dto.output_pdf_path} ({pdf_size_bytes} bytes)"
            ),
        )
