...
@final
class McpMediaController:
    """MCP Server controller for Media operations (image and audio generation)."""
    ...
    def __init__(self) -> None:
        ...
        self._mcp_server = Server(McpServerNameEnum.MEDIA.value)
    ...
    def __register_services_as_handlers(self) -> None:
        self._logger.log_info(
            module="mcp_media_controller", message="_register_handlers"
        )
        ...
        async def list_tools() -> list[Tool]:
            try:
                ...
                self.__log_exception(
                    module="mcp_media_controller._register_handlers.list_tools",
                    base_exception=e,
                )
                return []
        ...
        async def call_tool(event_name: str, payload_dict: dict) -> list[TextContent]:
            try:
                ...
                self.__log_exception(
                    module="mcp_media_controller._register_handlers.call_tool",
                    base_exception=e,
...
def start_mcp_or_fail() -> None:
    """Start the MCP server for Media operations."""
    try:
        asyncio.run(McpMediaController.get_instance()())
    except BaseException as error:
        Logger.get_instance().log_exception(
            module="mcp_media_controller.start_mcp",
