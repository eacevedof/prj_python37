...
class CreateListResultDto:
    """Salida del caso de uso CreateList.

    Cada caso de uso tiene SU propio DTO de resultado, aunque dos se parezcan.
    Compartirlos parece un ahorro y no lo es: el dia que crear tenga que devolver
    un campo que consultar no devuelve, o al reves, tendrias que tocar los dos
    caminos a la vez.

    `to_dict()` es lo que el controller mete en la clave `data` de la respuesta.
    Es el contrato con el cliente: si cambias una clave aqui, rompes al front.
    """
