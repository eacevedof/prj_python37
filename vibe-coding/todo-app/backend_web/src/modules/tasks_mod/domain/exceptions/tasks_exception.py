...
class TasksException(Exception):
    """Excepcion del modulo tasks. UNA por modulo, sin excepciones.
    ...
    `except TasksException` y saca el codigo de la propia excepcion.
    ...
    Sin `NoReturn`, esto daria un falso error de tipos:
        ...
        if list_row is None:
            TasksException.not_found_custom("no existe")
